...
/// @brief A generic ID to a Pinc object.
...
/// @return the newly completed window
...
typedef enum {
    ...
    /// @brief triggered when the mouse cursor moves in a window. Only triggered once per call to poll_events or wait_event.
...
///        Note that using this function still requires calling advance event. The timeout is in seconds, use Infinity or NaN for an infinite timeout.
...
extern void pinc_wait_event(float timeout);
